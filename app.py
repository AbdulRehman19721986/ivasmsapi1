"""
IVAS SMS Dashboard v3 — KEY FIX: Accept-Encoding = 'gzip, deflate' (no brotli) + binary file support
"""
...
class IVASClient:
    ...
    def _text(self, resp) -> str:
        """Robustly decode response, handling gzip only when content is actually gzipped."""
        ...
        if enc == 'gzip' and raw.startswith(b'\x1f\x8b'):
            try:
                ...
                logger.warning(f"gzip decompress failed: {e}, falling back to raw text")
                return resp.text
        elif enc == 'br':
            try:
                import brotli
                ...
                logger.warning("brotli not installed, using resp.text fallback")
                return resp.text
            except Exception as e:
                logger.warning(f"brotli decompress failed: {e}, using resp.text")
                return resp.text
        try:
    ...
    def fetch_numbers(self):
        ...
        try:
            ...
            html = self._text(resp)
            if "login" in html.lower():
                logger.warning("fetch_numbers: login page received, invalidating session")
                self.logged_in = False
                return None
            soup = BeautifulSoup(html, 'html.parser')
            out = []
    ...
    def fetch_received_stats(self, from_date='', to_date=''):
        ...
        try:
            ...
            html = self._text(resp)
            if "login" in html.lower():
                logger.warning("fetch_received_stats: login page received")
                self.logged_in = False
                return None
            soup = BeautifulSoup(html, 'html.parser')
    ...
    def fetch_numbers_in_range(self, phone_range, from_date='', to_date=''):
        ...
        try:
            ...
            html = self._text(resp)
            if "login" in html.lower():
                logger.warning("fetch_numbers_in_range: login page received")
                self.logged_in = False
                return []
            soup = BeautifulSoup(html, 'html.parser')
            out = []
    ...
    def fetch_otp_for_number(self, phone_number, phone_range, from_date='', to_date=''):
        ...
        try:
            ...
            html = self._text(resp)
            if "login" in html.lower():
                logger.warning("fetch_otp_for_number: login page received")
                self.logged_in = False
                return None
            soup = BeautifulSoup(html, 'html.parser')
    ...
    def fetch_live_sms(self):
        ...
        try:
            ...
            html = self._text(resp)
            if "login" in html.lower():
                logger.warning("fetch_live_sms: login page received")
                self.logged_in = False
                return None
            soup = BeautifulSoup(html, 'html.parser')
...
def debug_raw(p):
    """Proxy any file from IVAS, preserving original Content-Type and binary data."""
    ...
    # For text files, we might want to display inline; for binaries, download.
    # We'll serve as-is with the original Content-Type.
    return Response(
        r.content,
        status=r.status_code,
        headers={
            'Content-Type': content_type,
            'Content-Disposition': f'attachment; filename="{p.split("/")[-1]}"' if 'text' not in content_type else 'inline'
        }
    )
...
def debug_html(p):
    """Proxy HTML pages for debugging (plain text fallback)."""
    if not client.ensure_login():
        return "not logged in", 401
    r = client._req('GET', f"{BASE_URL}/{p}")
    if not r:
        return "no response", 500
    # For HTML, we return text but with original headers
