"""
IVAS SMS Dashboard v3 — FIXED: Accept-Encoding = 'gzip, deflate' + robust decompression
"""
...
class IVASClient:
    ...
    def _text(self, resp) -> str:
        ...
        raw = resp.content
        # If header says gzip but content doesn't start with gzip magic, treat as plain
        if enc == 'gzip' and raw.startswith(b'\x1f\x8b'):
            try:
                ...
                return resp.text
        # If we got brotli (should not happen, but handle gracefully)
        elif enc == 'br':
            try:
                import brotli
    ...
    def _verify(self) -> bool:
        try:
            ...
            if resp and resp.status_code == 200:
                ...
                # If we got the login page, session is dead
                if "login" in html.lower() or "_token" not in html:
    ...
    def fetch_numbers(self):
        ...
        try:
            ...
            if not out:
                # Fallback: extract all 10+ digit numbers
                seen = set()
